...
private:
    ...
    void handleWS(int fd, const std::string& req, const std::string& path) {
        ...
        // SDR++ DSP thread (audio) / IQ thread (fft) and drops any subscriber
        // whose sendAll() fails. Without a send timeout, a stuck/paused client
        // (socket buffer full, tab backgrounded, malicious hold) blocks that
        // send forever, stalling audio for EVERY listener and blocking sub/
        // unsub on subsMtx. SO_SNDTIMEO makes the send fail fast so the
        // existing drop logic evicts the laggard. A healthy LAN browser drains
        // continuously and never hits this; ~250ms caps the worst-case DSP
        // stall from one bad client before eviction.
